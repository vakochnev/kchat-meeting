...
class NotificationDispatcher:
    ...
    def _send_kchat(self, user: User, meeting: Meeting) -> bool:
        """Отправка уведомления в КЧАТ"""
        try:
            lines = [
                f"👋 Уважаемый(ая) {user.full_name},",
                "",
                "📢 Вы приглашены на оперативное совещание:",
                "",
            ...
            # Добавляем место только если указано
            if meeting.place and meeting.place.strip():
                lines.append(f"📍 Место: {meeting.place}")

            # Добавляем ссылку только если указана
            if meeting.link and meeting.link.strip():
                lines.append(f"🔗 Ссылка: {meeting.link}")

            lines.extend([
                "",
                "💬 Чтобы подтвердить участие:",
                "1️⃣ Введите команду /start",
                "2️⃣ После этого бот предложит выбрать:",
                "   • Да, буду присутствовать",
                "   • Нет, не смогу присутствовать"
            ])
